...
**無限スクロールに見えるが、実際は「次へ」のページャ**（`page_token=v1:N`）。
スクロールだけでは1ページ目の44件しか取れない。`--max-pages` で送ること。
`--scroll` は1ページ内の遅延読み込みに効くので併用する。
...
uv run --with playwright src/collect.py \
    ...
    --scroll 10 --max-pages 20 --delay 3 -o data/observations-mercari.csv
```

`--scroll` はページ高さが増えなくなった時点で、`--max-pages` は新規商品が
0件になった時点でそれぞれ打ち切るので、どちらも多めに指定してよい。
