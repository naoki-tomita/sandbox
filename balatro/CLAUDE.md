...
- **`App.tsx`** — the TUI counterpart of the web `App`: one `useState<GameEngine>` and an Ink `useInput` keyboard router that switches on `state.phase`. The hand is always shown rank-sorted (a user-pickable sort can come later). Selection is by number key (`1`-`8` toggle, indexing the *sorted* hand), `enter` plays (it runs `startPlay().resolvePlay()` back to back since there's no fly-off to wait for), `d` discards, the draft takes `1`-`3`/`s`, and `q`/`esc` quits. Sorting is display-only — the engine keys cards by id, so reordering never touches game state.
