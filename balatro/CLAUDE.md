...
- **`cards.ts`** — primitive types (`Suit`, `Rank`, `Card`) and pure functions (`createDeck`, `shuffle`, `cardChips`, `sortCards`). Card IDs are `"${rank}-${suit}"` (stable across shuffles). `sortCards(cards)` returns a copy sorted by rank (high → low) for readability — used by the TUI hand view.
...
- **`App.tsx`** — the TUI counterpart of the web `App`: a `useState<GameEngine>`, a `cursor` index into the displayed hand, and an Ink `useInput` keyboard router that switches on `state.phase`. The hand is always shown rank-sorted (a user-pickable sort can come later). In `selecting`, `←`/`→` move the cursor and `space` toggles the card under it, `enter` plays (it runs `startPlay().resolvePlay()` back to back since there's no fly-off to wait for), `d` discards; the draft takes `1`-`3`/`s`, and `q`/`esc` quits. Sorting and the cursor are display-only — the engine keys cards by id, so neither touches game state.
...
- Visual identity: **Neumorphism (soft UI)** — one cool-gray surface where body and panels share the same colour (`--paper` == the body background), sculpted purely by a top-left light and bottom-right shadow. The extrusion is driven by reusable shadow tokens in `global.css`: `--neu-raised` / `--neu-raised-sm` / `--neu-raised-lg` (lifted) and `--neu-pressed` / `--neu-pressed-sm` (recessed grooves), built from `--shadow-light` (#ffffff) and `--shadow-dark`. Colour tokens stay semantically named for the game: `--ink` (slate text), `--lacquer` (mult, soft red), `--cardback-blue` (chips, soft blue), `--gilt` (soft decorative accent). Chips are always blue, mult is always red. Display face is Poppins (`--font-display`), body is Nunito (`--font-body`), both loaded from Google Fonts in `index.html`. The worldview/theme may follow this restyle later. **Borders are mostly avoided** — depth comes from shadows, not outlines; raised elements use `--neu-raised*`, recesses (the hand tray, progress groove, empty joker slots) use `--neu-pressed*`.
